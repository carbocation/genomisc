...
func PrintCSVRow(row bulkprocess.DicomOutput, results chan<- string) error {
	...
	results <- fmt.Sprintf("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%.8f\t%d\t%d\t%d\t%d\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%s\t%s\t%s\t%s\t%s\t%v\t%s\t%s\t%s\t%s\t%s",
		row.SampleID,
		row.FieldID,
		row.Instance,
		row.Index,
		row.ZipFile,
		row.Filename,
		NA(row.DicomMeta.SeriesDescription),
		studyDate.Format("2006-01-02"),
		NA(row.DicomMeta.InstanceNumber),
		overlayText,
		...
		NA(row.DicomMeta.SeriesNumber),
		NA(row.DicomMeta.AcquisitionNumber),
		NA(row.DicomMeta.DeviceSerialNumber),
		NA(row.DicomMeta.StationName),
		NA(row.DicomMeta.SoftwareVersions),
		NA(row.DicomMeta.EchoTime),
		NA(row.DicomMeta.NominalInterval),
		NA(row.DicomMeta.SliceLocation),
		NA(row.DicomMeta.TriggerTime),
		NA(row.DicomMeta.AcquisitionTime),
