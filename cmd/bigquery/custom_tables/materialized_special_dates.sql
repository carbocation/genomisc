WITH dated_fields AS (
  SELECT p.FieldID, p.sample_id eid, p.value code, cod.meaning,
    ...
    END vdate
  FROM `broad-ml4cvd.jpp_ingest_test.phenotype` p
  JOIN `broad-ml4cvd.jpp_ingest_test.phenotype` denroll ON denroll.FieldID=53 AND denroll.sample_id=p.sample_id AND denroll.instance = 0 AND denroll.array_idx = 0
  JOIN `broad-ml4cvd.jpp_ingest_test.phenotype` d ON d.sample_id=p.sample_id AND d.instance = p.instance AND d.array_idx = p.array_idx
    AND (
      FALSE
  ...
  LEFT JOIN `broad-ml4cvd.jpp_ingest_test.coding` cod ON cod.coding_file_id = d.coding_file_id AND cod.coding = d.value
), 
