WITH oper4 AS (
  SELECT 41200 FieldID, eid, oper4 code, 
    CASE 
      ...
      ELSE h.epistart
    END vdate
  FROM `broad-ml4cvd.jpp_ingest_test.hesin` h
  ...
  SELECT 41210 FieldID, h.eid, sec.oper4 code, 
    CASE 
      ...
      ELSE h.epistart
    END vdate
  FROM `broad-ml4cvd.jpp_ingest_test.hesin_oper` sec 
  LEFT JOIN `broad-ml4cvd.jpp_ingest_test.hesin` h ON sec.eid=h.eid AND sec.record_id=h.record_id
  WHERE TRUE
  ...
  SELECT 41204 FieldID, h.eid, sec.diag_icd10 code, 
    CASE 
      ...
      ELSE h.epistart
    END vdate
  FROM `broad-ml4cvd.jpp_ingest_test.hesin_diag10` sec 
  LEFT JOIN `broad-ml4cvd.jpp_ingest_test.hesin` h ON sec.eid=h.eid AND sec.record_id=h.record_id
  WHERE TRUE
  ...
  SELECT 41205 FieldID, h.eid, sec.diag_icd9 code, 
    CASE 
      ...
      ELSE h.epistart
    END vdate
  FROM `broad-ml4cvd.jpp_ingest_test.hesin_diag9` sec
  LEFT JOIN `broad-ml4cvd.jpp_ingest_test.hesin` h ON sec.eid=h.eid AND sec.record_id=h.record_id
  WHERE TRUE
...
JOIN `broad-ml4cvd.jpp_ingest_test.phenotype` p ON p.sample_id = diagnostics.eid AND p.array_idx=0 AND p.instance=0 AND p.FieldID=53
