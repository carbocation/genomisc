...
func TestHWEFast(t *testing.T) {
	Cutoff := 1e-6
	for _, v := range examples {
		if p, expected := Fast(float64(v.AA), float64(v.Aa), float64(v.aa), Cutoff), v.P; p < Cutoff && math.Abs(p-expected) > Cutoff/2 {
			t.Fatalf("\nApprox: Error with input: %+v\nP: %.12f\nExpected: %.12f\nDiff: %.12f\n", v, p, expected, p-expected)
		}
	}
}
